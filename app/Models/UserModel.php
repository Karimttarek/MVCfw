...
class UserModel extends Model
{
    ...
    {
        return self::get(
        'SELECT au.*, aug.GroupName GroupName FROM ' . self::$table . ' au INNER JOIN app_users_groups aug ON aug.GroupId = au.GroupId WHERE au.UserId != ' . $user->UserId
        );
    }
    ...
    public static function authenticate ($username, $password, $session)
    {
        ...
        // $sql = 'SELECT *, (SELECT GroupName FROM app_users_groups WHERE app_users_groups.GroupId = ' . self::$table . '.GroupId) GroupName FROM ' . self::$table . ' WHERE Username = "' . $username . '" AND Password = "' .  $password . '"';
