...
use PHPMVC\App\Lib\Database\DatabaseHandler;
...
class Model
{
    ...
    {
        foreach (static::Schema as $columnName => $type) {
            if ($type == 4) {
    ...
    {
        $namedParams = '';
        foreach (static::$table as $columnName => $type) {
    ...
    {
        $sql = 'INSERT INTO ' . static::$table . ' SET ' . $this->buildNameParametersSQL();
    ...
    {
        $sql = 'UPDATE ' . static::$table . ' SET ' . $this->buildNameParametersSQL() . ' WHERE ' . static::$primaryKey . ' = ' . $this->{static::$primaryKey};
        $stmt = DatabaseHandler::factory()->prepare($sql);
        $this->prepareValues($stmt);
        return $stmt->execute();
    }
    ...
    public static function getByPK($pk)
    {
        ...
        if ($stmt->execute() === true) {
            if(method_exists(get_called_class(), '__construct')) {
                $obj = $stmt->fetchAll(\PDO::FETCH_CLASS | \PDO::FETCH_PROPS_LATE, get_called_class(), array_keys(static::Schema));
            } else {
